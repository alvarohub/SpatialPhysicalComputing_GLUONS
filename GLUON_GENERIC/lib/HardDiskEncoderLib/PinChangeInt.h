...
typedef void (*PCIntFuncPtr)(void* arg);
...
public:
	...
	static		int8_t attachInterrupt(uint8_t pin, PCIntFuncPtr userFunc, void* arg, int mode);
...
protected:
	class PCintPin {
	public:
		PCintPin() : PCintFunc((PCIntFuncPtr)NULL), mode(0) {}
		PCIntFuncPtr PCintFunc;
                void*           arg;
		uint8_t 	mode;
		uint8_t		mask;
		uint8_t arduinoPin;
	...
	void 		enable(PCintPin* pin, PCIntFuncPtr userFunc, void* userArg, uint8_t mode);
	int8_t		addPin(uint8_t arduinoPin,PCIntFuncPtr userFunc, void* userArg, uint8_t mode);
...
void PCintPort::enable(PCintPin* p, PCIntFuncPtr userFunc, void* userArg, uint8_t mode) {
...
int8_t PCintPort::addPin(uint8_t arduinoPin, PCIntFuncPtr userFunc, void* userArg, uint8_t mode)
{
	PCintPin* tmp;
	...
	if (firstPin != NULL) {
		tmp=firstPin;
		do {
			if (tmp->arduinoPin == arduinoPin) { enable(tmp, userFunc, userArg, mode); return(0); }
	...
	enable(p, userFunc, userArg, mode);
#ifdef DEBUG
 ...
 */
int8_t PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntFuncPtr userFunc, void* arg, int mode)
{
	PCintPort *port;
	...
	return(port->addPin(arduinoPin,userFunc,arg,mode));
}
